# You can use your custom user model if applicable
...
class Applicant(models.Model):
    ...
    applicant_purpose = models.CharField(
        max_length=30, choices=APPLICANT_PURPOSE_CHOICE, null=True, blank=True)
    logo = models.ImageField(upload_to='applicant_images/', blank=True)
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='Created', null=True, blank=True)
    ...
    academic_score_category = models.CharField(
        max_length=20, choices=ACADEMIC_SCORE_CHOICE, null=True, blank=True)
    academic_score = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    address = models.CharField(max_length=150, null=True, blank=True)
    ielts_score = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    toefl_score = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    pte_score = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    gre_score = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    gmat_score = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    sat_score = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    other_language = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    interested_country = models.ForeignKey(
        Country, on_delete=models.CASCADE, null=True, blank=True)
    interested_course = models.ForeignKey(
        Course, on_delete=models.CASCADE, null=True, blank=True)
    documents = models.ManyToManyField(
        Document, related_name='documents', blank=True)
    interested_institution = models.ForeignKey(
...
class Payment(models.Model):
    ...
    applicant = models.ForeignKey(
        Applicant, on_delete=models.CASCADE, related_name='applicant_payment')
...
class Invoice(models.Model):
    applicant = models.ForeignKey(
        Applicant, on_delete=models.CASCADE, related_name='applicant_invoice')
